...
__doc__ = "Groove class with dialog"
...
class Grooving_Form(QtGui.QWidget):
   ...
   def __init__(self, fr_obj) :
      ...
      dispo.addWidget(QtGui.QLabel("groove_depth", self),2,0)        
      self.Depth = ui.createWidget("Gui::QuantitySpinBox")
      self.Depth.setToolTip("the depth of the groove")
      ...
      dispo.addWidget(QtGui.QLabel("Start X", self),4,0)        
      self.StartX = ui.createWidget("Gui::QuantitySpinBox")
      self.StartX.setToolTip("Start X of the groove")
      ...
      dispo.addWidget(QtGui.QLabel("Start Y", self),5,0)        
      self.StartY = ui.createWidget("Gui::QuantitySpinBox")
      self.StartY.setToolTip("Start Y of the groove")
      ...
      dispo.addWidget(QtGui.QLabel("angle and length", self),6,0)        
      self.Winkel_und_Laenge = QtGui.QCheckBox(self)
      dispo.addWidget(self.Winkel_und_Laenge, 6, 1)
      self.Winkel_und_Laenge.setToolTip("if checked, give angle and length,\nif mode is \"through\" without function")
      ...
      dispo.addWidget(QtGui.QLabel("Length", self),8,0)        
      self.Laenge = ui.createWidget("Gui::QuantitySpinBox")
      self.Laenge.setToolTip("Length of the groove")
      ...
      dispo.addWidget(QtGui.QLabel("End X", self),9,0)        
      self.EndX = ui.createWidget("Gui::QuantitySpinBox")
      self.EndX.setToolTip("End X of the groove")
      self.EndX.setProperty("minimum", 0.0)
      self.EndX.setProperty("maximum", 5000.0)
      self.EndX.setProperty("setSingleStep" , 0.1)
      dispo.addWidget(self.EndX, 9, 1)   
      self.EndX.setProperty('rawValue',fr_obj.EndX)      
      Gui.ExpressionBinding(self.EndX).bind(fr_obj,"EndX")

      dispo.addWidget(QtGui.QLabel("End Y", self),10,0)        
      self.EndY = ui.createWidget("Gui::QuantitySpinBox")
      self.EndY.setToolTip("End Y of the groove")
      self.EndY.setProperty("minimum", 0.0)
      self.EndY.setProperty("maximum", 5000.0)
      self.EndY.setProperty("setSingleStep" , 0.1)
      dispo.addWidget(self.EndY, 10, 1)   
      self.EndY.setProperty('rawValue',fr_obj.EndY)      
      Gui.ExpressionBinding(self.EndY).bind(fr_obj,"EndY")

      dispo.addWidget(QtGui.QLabel('side left or right', self), 11, 0)
      ...
      self.RadiusCorrection.setToolTip("defines on which side or in the middle")
      ...
      self.Mode.setToolTip("defines where is the startpoint and endpoint; on the ground or surface,\ndepends on depth and tool radius,\n through goes totally through the workpiece")
...
class Grooving_Form_TaskPanel(QtGui.QWidget):
   ...
   def on_EndX_valueChanged(self,state):
      self.fr_obj.EndX=float(state)
      self.fr_obj.Document.recompute()  
      App.ActiveDocument.recompute()  
   def on_EndY_valueChanged(self,state):
      self.fr_obj.EndY=float(state)
      self.fr_obj.Document.recompute()  
      App.ActiveDocument.recompute()  
   def on_Depth_valueChanged(self,state):
      self.fr_obj.Depth=int(float(state))
      self.fr_obj.Document.recompute()  
      App.ActiveDocument.recompute()  
...
class Grooving:
   ...
   def __init__(self, obj,spx,spy,epx,epy,depth,wi,rc,dir,cond,angle,com,mode,bid):
        self.Type = 'Nut'
        obj.Proxy = self
        obj.addProperty('App::PropertyString', 'Description', 'Base', 'Box description').Description = "groove"
        obj.addProperty('App::PropertyFloat', 'Grooving_blade_radius', 'Dimensions', 'Grooving_blade_radius')
